...
//debecho('<br /><br />I received this now: '.time()."<br />"); print_r($_REQUEST);


if ($nossl_config['private_key_auto_change']=="on" && $nossl_config['private_key_change_interval'] < time() - $nossl_rsa_timestamp){ //If the time for this NOSSL key has run out, make a new one!
    //debecho('<br />In if private_key_auto_change');
    $extra_nossl = new NoSSL();
    $extra_nossl->createNewRSAKey(2048); //A new RSA key is generated and stored in /nossl/nossl_config/RSA_privatekey.php and RSA_publickey.php    
}
...
function serverSettings2JSON($handshake='1'){
    global $nossl, $nossl_config;
    $itemsarray = array();
    $remainingLeaseTime = $GLOBALS['nossl_rsa_timestamp'] + $nossl_config['private_key_change_interval'] - time(); //What is the remaining validity time of the current RSA key? Tell it to the client so he can refresh accordingly
